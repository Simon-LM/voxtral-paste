#!/bin/bash
# VoxRefiner — Selection to Fact-check
# Select text, then fact-check directly — no summary generated upfront.
# Press [Enter] to fact-check the whole text, or dictate/type a specific claim.
...
if [ -z "$(printf '%s' "$selected_text" | tr -d '[:space:]')" ]; then
    _header "SELECTION TO FACT-CHECK" "⌨→🔬"
    echo ""
...
INSIGHT_DIR="$SCRIPT_DIR/recordings/factcheck"
mkdir -p "$INSIGHT_DIR"
...
_SETTING_TRANSLATE_LANG="${TRANSLATE_TARGET_LANG:-en}"

# ─── Step 1: Fact-check directly ─────────────────────────────────────────────

_factcheck_flow "$selected_text"
...
while true; do
    echo ""
    _sep
    _menu_line="  ${C_BOLD}[f]${C_RESET} New fact-check  ${C_BOLD}[p]${C_RESET} Search  ${C_BOLD}[z]${C_RESET} Summarise  ${C_BOLD}[l]${C_RESET} Read full"
    [ "$_factcheck_done" -eq 1 ] && _menu_line="$_menu_line  ${C_BOLD}[r]${C_RESET} Replay fact-check"
    ...
    case "$_main_action" in
        ...
        r|R) [ "$_factcheck_done" -eq 1 ] && _play_audio "$FACTCHECK_AUDIO" ;;
        ...
        d|D)
            if   [ "$_article_done"   -eq 1 ] && [ -f "$FULL_ARTICLE_AUDIO" ]; then
                _save_audio_to_downloads "$FULL_ARTICLE_AUDIO" "$selected_text" "factcheck-full-article"
            elif [ "$_factcheck_done" -eq 1 ] && [ -f "$FACTCHECK_AUDIO" ]; then
                _save_audio_to_downloads "$FACTCHECK_AUDIO" "$selected_text" "factcheck-result"
            elif [ "$_search_done"    -eq 1 ] && [ -f "$SEARCH_AUDIO" ]; then
                _save_audio_to_downloads "$SEARCH_AUDIO" "$selected_text" "factcheck-search"
            elif [ "$_summary_done"   -eq 1 ] && [ -f "$SUMMARY_AUDIO" ]; then
                _save_audio_to_downloads "$SUMMARY_AUDIO" "$selected_text" "factcheck-summary"
            else
